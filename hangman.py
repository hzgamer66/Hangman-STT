...
def play_hangman():
    ...
    while lives > 0:
        ...
        time.sleep(1)
        try:
            guessed_letter = recordAndConvert.get_letter().lower()
        except:
        ...
        if guessed_letter == 'help':  # extra credit part 1 (determine if help was typed)
            ...
            time.sleep(1)
            try:
                answer_guess = recordAndConvert.get_letter().lower()
            except:
                print('')
...
def play_again():  # asks if you want to play again or not
    while True:
        ...
        time.sleep(1)
        try:
            play_again_response = recordAndConvert.get_letter().lower()
        except:
            print('Invalid input.')
            continue
